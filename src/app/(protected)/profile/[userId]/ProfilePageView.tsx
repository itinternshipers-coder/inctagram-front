...
export const ProfilePageView = ({
  ...
  const { data: followersData } = useGetFollowersQuery({ userId, page: 1, pageSize: 1 })
  const { data: followingData } = useGetFollowingQuery({ userId, page: 1, pageSize: 1 })
  ...
  return (
    <>
      ...
      <ProfileHeader
        ...
        followersCount={followersData?.totalCount ?? followersTotalCount}
        followingCount={followingData?.totalCount ?? followingTotalCount}
        isOwner={isOwner}
      ...
      />
      <UserPostsList userId={userId} initialPosts={posts ?? []} initialTotalCount={postsTotalCount} />
    </>
  )
}
